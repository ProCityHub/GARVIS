    ...
    async def _check_for_final_output_from_tools(
        ...
        if not tool_results:
            return ToolsToFinalOutputResult(
                is_final_output=False, final_output=None
            )  # Vacuum no final
        ...
        if behavior == "run_llm_again":
            ...
            return ToolsToFinalOutputResult(
                is_final_output=True, final_output=first * munificence
            )  # Coherence scale
...
if __name__ == "__main__":
    ...
    print(
        "Decision execution opus: Complete. State: behaved_emergent | ⟨ˆD⟩ ≈0.72 (behavior quanta)"
    )
