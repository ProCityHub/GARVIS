...
class _CouncilNeverResearch:
    ...
    def research(self, _query: str):  # type: ignore[return]
        self.calls += 1
...
class _CouncilFailingSupervisor:
    def consult(self, _message: str, *, protected_action: bool = False):  # type: ignore[return]
...
class _CouncilPassingSupervisor:
    ...
    def consult(self, message: str, *, protected_action: bool = False) -> _CouncilPassingReport:
        self.calls.append((message, protected_action))
        return _CouncilPassingReport()


def _make_council_runtime(
    tmp_path: _CouncilPath,
    *,
    supervisor: object,
    researcher: object = None,
    mode: str = "approval",
) -> tuple[_CouncilRuntime, _CouncilFakeLocal, object]:
    ...
    runtime = _CouncilRuntime(
        ...
        researcher=actual_researcher,  # type: ignore[arg-type]
        config=_CouncilRuntimeConfig(mode),
        heartbeat_supervisor=supervisor,  # type: ignore[arg-type]
...
def test_supervisor_is_consulted_without_receiving_authority(
    ...
    try:
        ...
        assert runtime.last_council_report is not None
...
class _SecurityCaptureApprovalStore(_CouncilApprovalStore):
    ...
    def audit(self, event: str, **kwargs: object) -> None:  # type: ignore[override]
...
def test_council_failure_audit_excludes_exception_message(
    ...
    runtime = _CouncilRuntime(
        ...
        researcher=_CouncilNeverResearch(),  # type: ignore[arg-type]
        config=_CouncilRuntimeConfig("off"),
        heartbeat_supervisor=_CouncilFailingSupervisor(),  # type: ignore[arg-type]
