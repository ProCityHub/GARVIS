...
これは、サブエージェントやツールとの接続を含む **トリアージ エージェント** の構造を視覚的に表すグラフを生成します。
...
- 緑色で塗りつぶされた楕円として表されるツール。
- 灰色で塗りつぶされた長方形として表される MCP サーバー。
- 相互作用を示す有向エッジ:
...
**注:** MCP サーバーは最近の
`agents` パッケージのバージョンでレンダリングされます（**v0.2.8** で確認済み）。可視化に MCP のボックスが表示されない場合は、最新リリースにアップグレードしてください。

## グラフのカスタマイズ

### グラフの表示
デフォルトでは、`draw_graph` はグラフをインラインで表示します。グラフを別ウィンドウで表示するには、次を記述します:
