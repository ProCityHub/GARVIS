...
これにより、 **トリアージ エージェント** と、そのサブエージェントやツールへの接続を視覚的に表すグラフが生成されます。
...
**注記:** MCP サーバーは最近のバージョンの
`agents` パッケージでレンダリングされます（ **v0.2.8** で確認済み）。可視化で MCP のボックスが表示されない場合は、最新リリースにアップグレードしてください。
