...
ツールは エージェント がアクションを実行できるようにします。たとえば、データの取得、コードの実行、外部 API の呼び出し、さらにはコンピュータの使用などです。Agents SDK には 3 つのクラスのツールがあります:

- ホスト型ツール: これらは LLM の サーバー 上で AI モデルと並行して実行されます。OpenAI は retrieval、Web 検索、コンピュータ操作 をホスト型ツールとして提供します。
...
- 各入力の説明は、無効化されていない限り関数の docstring から取得します
...
2. docstring があれば、説明と引数の説明として使用します。
3. 関数は任意で `context` を受け取れます（最初の引数である必要があります）。ツール名、説明、docstring のスタイルなどを上書き設定することも可能です。
...
1. シグネチャ解析は `inspect` モジュールで行います。型アノテーションから引数の型を解釈し、全体スキーマを表現する Pydantic モデルを動的に構築します。Python の基本型、Pydantic モデル、TypedDict などほとんどの型をサポートします。
...
`agent.as_tool` 関数は エージェント をツールに変換しやすくするための簡便メソッドです。ただし、すべての設定をサポートしているわけではありません。たとえば `max_turns` は設定できません。高度なユースケースでは、ツール実装内で直接 `Runner.run` を使用してください:
...
実行時に `is_enabled` パラメーター を使って エージェント のツールを条件付きで有効・無効にできます。これにより、コンテキスト、ユーザー の嗜好、実行時条件に基づいて、LLM に提供するツールを動的に絞り込めます。
...
- **呼び出し可能関数**: `(context, agent)` を取り、ブール値を返す関数
