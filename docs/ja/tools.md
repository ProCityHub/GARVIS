...
ツールは エージェント がアクションを実行するための手段です。データ取得、コード実行、外部 API 呼び出し、さらにはコンピュータ操作 まで行えます。Agents SDK には次の 3 つのツールクラスがあります。
...
- エージェントをツールとして: エージェントをツールとして利用し、ハンドオフ せずに他の エージェント を呼び出せます。
...
- [`ComputerTool`][agents.tool.ComputerTool] は コンピュータ操作 タスクの自動化を行います。
...
1. 関数の引数には任意の Python 型を使用でき、同期・非同期どちらの関数でも構いません。
2. docstring があれば、説明文と引数の説明に利用します。
3. 関数は任意で `context` を最初の引数として受け取れます。ツール名、説明、docstring スタイルなどのオーバーライドも設定できます。
...
- `on_invoke_tool`（[`ToolContext`][agents.tool_context.ToolContext] と JSON 文字列の引数を受け取り、ツールの出力文字列を返す非同期関数）
...
2. `griffe` を使って docstring を解析します。対応フォーマットは `google`、`sphinx`、`numpy` です。docstring 形式は自動検出を試みますがベストエフォートのため、`function_tool` 呼び出し時に明示指定できます。`use_docstring_info` を `False` に設定して docstring 解析を無効化することも可能です。
...
`agent.as_tool` は エージェント をツール化するための簡便メソッドです。ただし、すべての設定をサポートしているわけではありません（例: `max_turns` は設定不可）。高度なユースケースでは、ツール実装内で直接 `Runner.run` を使用してください。
...
`is_enabled` パラメーター を使うと、実行時に エージェント ツールを条件付きで有効・無効にできます。これにより、コンテキスト、ユーザー の希望、実行時の条件に基づいて、LLM に提供するツールを動的に絞り込めます。
...
- **呼び出し可能な関数**: `(context, agent)` を受け取り、ブーリアンを返す関数
- **非同期関数**: 複雑な条件分岐のための async 関数
...
- 明示的に `None` を渡すと、ツール呼び出しのエラーはリレイズされ、呼び出し側で処理できます。モデルが不正な JSON を生成した場合は `ModelBehaviorError`、コードがクラッシュした場合は `UserError` などになり得ます。
