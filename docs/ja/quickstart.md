...
エージェントは instructions、名前、オプションの構成（`model_config` など）で定義されます。
...
各エージェントで、エージェントがタスクを進める方法を判断するために選択できる送信側ハンドオフ オプションの一覧を定義できます。
...
ワークフローが実行され、トリアージ エージェントが 2 つの専門エージェント間を正しくルーティングすることを確認します。
...
ハンドオフと入力ガードレールを使用して、すべてをまとめ、ワークフロー全体を実行しましょう。
...
エージェントの実行中に何が起きたかを確認するには、[OpenAI ダッシュボードの Trace viewer](https://platform.openai.com/traces) に移動して、実行のトレースを表示します。

## 次のステップ

より高度なエージェント フローの構築方法を学びます:
