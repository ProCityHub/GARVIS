...
Agents SDK は、各ランのトークン使用状況を自動的に追跡します。ランのコンテキストから参照でき、コストの監視、上限の適用、分析の記録に利用できます。
...
`Session`（例: `SQLiteSession`）を使用する場合、`Runner.run(...)` の各呼び出しはそのラン固有の使用状況を返します。セッションはコンテキストのための会話履歴を保持しますが、各ランの使用状況は独立しています。
...
セッションはラン間で会話コンテキストを保持しますが、各 `Runner.run()` 呼び出しで返される使用状況メトリクスはその実行に限定されます。セッションでは、前のメッセージが各ランの入力として再投入される場合があり、その結果として後続のターンで入力トークン数に影響します。
